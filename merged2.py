...
# -*- coding: utf-8 -*-
"""
Created on Tue May 17 11:19:22 2022
...
def set_scene(mesh, homo_trans, cam_intrin):
    # set the scene with pyrender
    scene = pyrender.Scene()
    camera = pyrender.IntrinsicsCamera(fx=cam_intrin.fx, fy=cam_intrin.fy,
                                       cx=cam_intrin.ppx, cy=cam_intrin.ppy)
    chessboard = pyrender.Node(mesh=mesh, matrix=homo_trans)
    cam = pyrender.Node(camera = camera, matrix=np.eye(4))
    scene.add_node(chessboard)
    scene.add_node(cam,parent_node=chessboard)
...
def main():
    ...
    while True:
        ...
        if flag == False:
            scene, chessboard = set_scene(mesh, homo_trans, color_intrin)
            flag = True
        else:
